# API

REST API для управления тикетами.

## Базовый URL

```
http://localhost:8000/api
```

## Эндпоинты

### Health Check

```
GET /api/health
```

**Ответ (200):**
```json
{ "status": "ok" }
```

---

### Авторизация

#### Вход

```
POST /api/auth/login
```

**Тело запроса:**
```json
{
  "username": "admin",
  "password": "admin"
}
```

**Ответ (200):**
```json
{
  "access_token": "eyJ...",
  "token_type": "bearer"
}
```

**Ошибки:**
- `401` — Неверные учётные данные
- `429` — Превышен лимит запросов (10/мин)

#### Верификация токена

```
GET /api/auth/verify
```

**Заголовки:**
```
Authorization: Bearer <token>
```

**Ответ (200):**
```json
{ "valid": true }
```

**Ошибки:**
- `401` — Требуется заголовок авторизации
- `401` — Неверный заголовок авторизации
- `401` — Неверный или истёкший токен

---

### Тикеты

#### Создание

```
POST /api/tickets
```

**Тело запроса:**
```json
{
  "title": "Название тикета",
  "description": "Описание (необязательно)",
  "priority_id": 2
}
```

**Поля:**
| Поле | Тип | Обязательно | Описание |
|------|-----|-------------|----------|
| `title` | string | да | 3–120 символов |
| `description` | string | нет | До 1000 символов |
| `priority_id` | int | нет | ID приоритета (по умолчанию 2 = normal) |

**Ответ (201):**
```json
{
  "id": 1,
  "title": "Название тикета",
  "description": "Описание",
  "status": "new",
  "priority_id": 2,
  "priority_name": "normal",
  "created_at": "2026-06-28T12:00:00Z",
  "updated_at": "2026-06-28T12:00:00Z"
}
```

---

#### Список (с фильтрами, сортировкой, пагинацией)

```
GET /api/tickets
```

**Параметры запроса:**
| Параметр | Тип | По умолчанию | Описание |
|----------|-----|--------------|----------|
| `status` | string | — | Фильтр по статусу: `new`, `in_progress`, `done` |
| `priority_id` | int | — | Фильтр по ID приоритета (1=low, 2=normal, 3=high) |
| `search` | string | — | Поиск по названию и описанию |
| `sort_by` | string | `created_at` | Сортировка: `created_at`, `priority` |
| `sort_order` | string | `desc` | Направление: `asc`, `desc` |
| `page` | int | `1` | Номер страницы (≥1) |
| `page_size` | int | `20` | Элементов на странице (1–100) |

**Пример:**
```
GET /api/tickets?status=new&priority_id=3&sort_by=created_at&sort_order=desc&page=1&page_size=10
```

**Ответ (200):**
```json
{
  "items": [
    {
      "id": 1,
      "title": "Тикет",
      "description": null,
      "status": "new",
      "priority_id": 3,
      "priority_name": "high",
      "created_at": "2026-06-28T12:00:00Z",
      "updated_at": "2026-06-28T12:00:00Z"
    }
  ],
  "total": 42,
  "page": 1,
  "page_size": 10,
  "total_pages": 5
}
```

---

#### Получение по ID

```
GET /api/tickets/{id}
```

**Ответ (200):** Тикет (см. выше)

**Ошибки:**
- `404` — Тикет с id {id} не найден

---

#### Обновление

```
PATCH /api/tickets/{id}
```

**Тело запроса:**
```json
{
  "title": "Новое название",
  "description": "Новое описание",
  "priority_id": 3
}
```

Все поля необязательные. Обновляются только переданные.

**Ответ (200):** Обновлённый тикет

**Ошибки:**
- `400` — Действие не разрешено для текущего статуса
- `404` — Тикет с id {id} не найден

---

#### Изменение статуса

```
PATCH /api/tickets/{id}/status
```

**Тело запроса:**
```json
{
  "status": "in_progress"
}
```

**Правила (Action Matrix):**
| Текущий статус | Действие `transition` | Доступные переходы |
|----------------|----------------------|-------------------|
| `new` | ✅ разрешено | `new`, `in_progress`, `done` |
| `in_progress` | ✅ разрешено | `new`, `in_progress`, `done` |
| `done` | ❌ запрещено | `done` (no-op) |

**Ответ (200):** Обновлённый тикет

**Ошибки:**
- `400` — Действие не разрешено для текущего статуса

---

#### Удаление

```
DELETE /api/tickets/{id}
```

**Заголовки (требуется для admin):**
```
Authorization: Bearer <token>
```

**Ответ (204):** Нет тела

**Ошибки:**
- `400` — Действие не разрешено для текущего статуса
- `403` — Требуется доступ администратора
- `404` — Тикет с id {id} не найден

---

### Приоритеты

#### Список приоритетов

```
GET /api/priorities
```

**Ответ (200):**
```json
[
  { "id": 1, "name": "low", "sort_order": 0 },
  { "id": 2, "name": "normal", "sort_order": 1 },
  { "id": 3, "name": "high", "sort_order": 2 }
]
```

#### Создание приоритета

```
POST /api/priorities
```

**Заголовки (требуется admin):**
```
Authorization: Bearer <token>
```

**Тело запроса:**
```json
{
  "name": "critical",
  "sort_order": 3
}
```

**Ответ (201):**
```json
{
  "id": 4,
  "name": "critical",
  "sort_order": 3
}
```

#### Обновление приоритета

```
PATCH /api/priorities/{id}
```

**Заголовки (требуется admin):**
```
Authorization: Bearer <token>
```

**Тело запроса:**
```json
{
  "name": "urgent",
  "sort_order": 3
}
```

**Ответ (200):** Обновлённый приоритет

**Ошибки:**
- `404` — Приоритет с id {id} не найден

#### Удаление приоритета

```
DELETE /api/priorities/{id}
```

**Заголовки (требуется admin):**
```
Authorization: Bearer <token>
```

**Ответ (204):** Нет тела

**Ошибки:**
- `404` — Приоритет с id {id} не найден
- `400` — Приоритет используется тикетами (FK constraint)

---

## Коды ответов

| Код | Описание |
|-----|----------|
| `200` | Успешный запрос |
| `201` | Ресурс создан |
| `204` | Успешное удаление (нет тела) |
| `400` | Ошибка бизнес-правил |
| `401` | Не авторизован |
| `403` | Нет доступа |
| `404` | Ресурс не найден |
| `422` | Ошибка валидации (Pydantic) |
| `429` | Превышен лимит запросов |
